...
//    涉及到的方法有：求最大值、最小值、总和、平均数、遍历数组、复制数组、数组反转、数组排序(默认从小到大排序)、查找等
    ...
    /**
     * @param arr        要排列的数组
     * @param sortMethod 指定排序方式，sc为升序，desc为降序
     */
    public void sort(int[] arr, String sortMethod) {// 数组排序，从小到大
        if (arr != null && arr.length != 0) {
            for (int i = arr.length - 1; i > 0; i--) {
                if ("asc".equals(sortMethod)) {// 升序排列
                    for (int j = 0; j < i; j++) {
                        if (arr[j] > arr[j + 1]) {
                            swap(arr, j, j + 1);
                        }
                    }
                } else if ("desc".equals(sortMethod)) {// 降序排列
                    for (int j = 0; j < i; j++) {
                        if (arr[j] < arr[j + 1]) {
                            swap(arr, j, j + 1);
                        }
